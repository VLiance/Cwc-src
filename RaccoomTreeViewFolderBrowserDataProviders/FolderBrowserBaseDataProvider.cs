...
using Raccoom.TreeViewFolderBrowser.DataProviders.Properties;
using System;
...
namespace Raccoom.Windows.Forms
{
	...
	public abstract class TreeStrategyShellImageListProviderBase  : TreeStrategyDataProviderBase, IDisposable
	{
        ...
        public  void InitializeIcon(){
           if(!bSet){
                bSet = true;
                fAddIcon(Helper.TreeView, Resources.application_link, "ProjectRoot" ) ;
                fAddIcon(Helper.TreeView, Resources.bug_link, "OtherRoot" );
                fAddIcon(Helper.TreeView, Resources.brick_link, "LibRoot" );
                fAddIcon(Helper.TreeView, Resources.package_link, "Lib" );
                fAddIcon(Helper.TreeView, Resources.database_link, "ToolChain");
                fAddIcon(Helper.TreeView, Resources.drive_link, "ToolChainRoot");
            }
        ...
        public void fAddIcon(TreeView treeView,  Icon _oIcon , string _sName ) {
            
            if( treeView.ImageList == null){
              treeView.ImageList = new System.Windows.Forms.ImageList();
                treeView.ImageList.Images.Add("",  Resources.link_break);
            }
            if (!treeView.ImageList.Images.ContainsKey(_sName)) {
 
                  treeView.ImageList.Images.Add(_sName,  _oIcon);
             
            }
         //   node.ImageKey =_sName;
         //   node.SelectedImageKey = _sName;

        }
        public void fSetIcon(TreeNodePath node,  Icon _oIcon , string _sName ) {
            
            fAddIcon(node.TreeView, _oIcon, _sName);
