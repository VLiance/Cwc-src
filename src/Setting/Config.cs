...
namespace cwc
{
	...
	public class Config : FileUtils{
        ...
        public void fLoad() {
            ...
            foreach (XmlNode _oNode in _oXml.DocumentElement.ChildNodes){ 
                switch (_oNode.Name) {
                    case "vStartPos":foreach( XmlAttribute _oAtt in _oNode.Attributes) {switch ( _oAtt.Name) {
                            case "X":{int _nOut = 0; Int32.TryParse( _oAtt.Value, out _nOut ); 
                                vStartPos.X = _nOut;}; break;
                            case "Y":{int _nOut = 0; Int32.TryParse( _oAtt.Value, out _nOut ); 
                                vStartPos.Y = _nOut;}; break;
                    }}break;
                    case "vStartSize":foreach( XmlAttribute _oAtt in _oNode.Attributes) {switch ( _oAtt.Name) {
                            case "Height":{int _nOut = 0; Int32.TryParse( _oAtt.Value, out _nOut ); 
                                vStartSize.Height = _nOut;}; break;
                            case "Width":{int _nOut = 0; Int32.TryParse( _oAtt.Value, out _nOut ); 
                                vStartSize.Width = _nOut;}; break;
                    }}break;
                    case "bTreePrjOpen":foreach( XmlAttribute _oAtt in _oNode.Attributes) {switch ( _oAtt.Name) {
                            case "Val":{ bTreePrjOpen = fTestBool(_oAtt.Value); }; break;
                    }}break;
                    case "bMaximize":foreach( XmlAttribute _oAtt in _oNode.Attributes) {switch ( _oAtt.Name) {
        ...
        private void fExtractElement(XmlNode _oNode) {
            ...
            foreach( XmlAttribute _oAtt in _oNode.Attributes) {
                switch ( _oAtt.Name) {
                    case "Name": _sName = _oAtt.Value;
                    break;
                    case "Val":  _sVal = _oAtt.Value;;
                    break;
                }
            }
        ...
        private void fExtractRecent(XmlNode _oNode) {
            foreach( XmlAttribute _oAtt in _oNode.Attributes) {
                switch ( _oAtt.Name) {
                    case "Path": aRecent.Add(_oAtt.Value);
                    break;
                }
            }
        }
	...
	public class ConfigMng
	{
		...
		public void LoadConfig(){
           ...
           oConfig.fSetNewFile(m_sConfigFileName);
