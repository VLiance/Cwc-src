...
using static Tester.Win32;
using System.Diagnostics;

namespace Tester
{
    public partial class CWide : Form
    {
        protected override void WndProc(ref Message m){
           Boolean handled = false; //m.Result = IntPtr.Zero;

           string _sMsg = cwc.Msg.fReceiveMsg(ref m);
            if(_sMsg != "") {
                handled = true;
               // Console.WriteLine("YEEEEE: " + _sMsg);
                Activate();
                Focus();
           ...
           // m.Result = Window_Proc(m.HWnd, m.Msg,m.WParam,m.LParam, ref handled);
            if (handled) DefWndProc(ref m); else base.WndProc(ref m);
            
        }

        public void fProcessMsg(string _sMsg) {
            string[] _aMsg = _sMsg.Split('|');
            string _sArg2 =_aMsg.Length>0? _aMsg[1] : "";
            string _sArg3 =_aMsg.Length>1? _aMsg[2] : "";
            string _sArg4 =_aMsg.Length>2? _aMsg[3] : "";
        ...
        Color currentLineColor = Color.FromArgb(10, 210, 210, 255);
        Color changedLineColor = Color.FromArgb(40, 230, 230, 255);
        ...
        private void collapsibleSplitter3_SplitterMoved(object sender, SplitterEventArgs e)
        {
        ...
        {
            Icon = new Icon(assembly.GetManifestResourceStream("Tester.Resources.Cwide.ico"));
            //Console.WriteLine("ProcId = "  + Process.GetCurrentProcess().Id);
            //Console.WriteLine("Handle = "  + Process.GetCurrentProcess().MainWindowHandle);
            //Console.WriteLine("FHandle = "  + Handle);
           // tsFiles.BackColor = Color.DarkOrange;
            tsFiles.BackColor = Color.FromArgb(208,88,37);
